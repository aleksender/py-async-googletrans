...
from googletrans.models import TranslateSession
...
class AsyncTokenAcquirer(TokenAcquirer):
    RE_TKK = re.compile(r'tkk:\'(.+?)\'', re.DOTALL)
    RE_RAWTKK = re.compile(r'tkk:\'(.+?)\'', re.DOTALL)

    def __init__(self, tkk: str = '0', session: TranslateSession = None, host: str = 'translate.google.com'):
        self.session = session or TranslateSession()
        self.tkk = tkk
        self.host = host if 'http' in host else 'https://' + host
    ...
    async def _update(self):
        ...
        raw_tkk = self.RE_TKK.search(r_text)
        if raw_tkk:
            self.tkk = raw_tkk.group(1)
            return

        # this will be the same as python code after stripping out a reserved word 'var'
        code = unicode(self.RE_TKK.search(r_text).group(1)).replace('var ', '')
        # unescape special ascii characters such like a \x3d(=)
        if PY3:  # pragma: no cover
            code = code.encode().decode('unicode-escape')
        else:  # pragma: no cover
